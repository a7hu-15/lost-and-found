...
- Name: public.spec.ts >> Public Website & Forms >> [U008] Submit Found report without image (Fails validation)
- Location: tests/public.spec.ts:84:7
...
Call log:
  - waiting for getByPlaceholder('e.g. Silver Macbook Pro 14-inch')
...
- main [ref=e3]:
  - paragraph [ref=e4]:
    ...
    - generic [ref=e9]:
      - text: "ID:"
      - code [ref=e10]: "`bom1::kds9p-1786961309956-3a5c41bd8450`"
...
```ts
  ...
  3   | 
  4   | test.describe('Public Website & Forms', () => {
  5   |   const dummyImage = path.join(__dirname, '../test-data/dummy.png');
  6   | 
  7   |   test('[U001] Homepage loads correctly', async ({ page }) => {
  8   |     await page.goto('/');
  9   |     await expect(page.locator('text=How can we help today?')).toBeVisible();
  10  |     await expect(page.getByRole('link', { name: 'Report Lost Item' })).toBeVisible();
