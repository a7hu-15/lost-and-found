    ...
    <div className="space-y-6 py-4">
      ...
      <div className="saas-card p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">Search Items</h1>
            <p className="text-xs text-zinc-400 mt-0.5">Search lost and found items by keywords or photo match</p>
          </div>
          ...
          <form onSubmit={handleSearch} className="space-y-3">
            <div className="relative">
              ...
              <input
                ...
                className="saas-input w-full py-2 pl-9 pr-3 text-xs font-mono"
              />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-1 font-mono">Category</label>
                <select
                  value={category}
                  onChange={(e) => setCategory(e.target.value)}
                  className="saas-input w-full py-1.5 px-2 text-xs font-mono"
                >
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-1 font-mono">Location</label>
                <input
                  type="text"
                  value={location}
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-1 font-mono">Color</label>
                <input
                  type="text"
                  value={color}
      ...
      <div className="space-y-6">
        <div>
          <h2 className="text-sm font-semibold text-white tracking-tight mb-3">
            Lost Reports ({loading ? '...' : lostItems.length})
          </h2>
          {loading ? (
            ...
            <div className="saas-card p-6 text-center text-xs text-zinc-400 font-mono space-y-1">
              <div className="text-zinc-200 font-semibold">No matching reports yet.</div>
              <div>New reports are added continuously. We'll also notify you automatically if a strong match is found.</div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {lostItems.map((item) => (
        ...
        <div>
          <h2 className="text-sm font-semibold text-white tracking-tight mb-3">
            Found Items ({loading ? '...' : foundItems.length})
          </h2>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <SkeletonCard />
