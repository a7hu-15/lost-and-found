...
def calculate_item_similarity(lost: LostItem, found: FoundItem) -> Tuple[float, Dict[str, Any]]:
    ...
    - Title / Item Name Similarity: 30 (Hard Gatekeeper)
    ...
    # 1. Title / Item Name Similarity (30 pts, acts as a soft gatekeeper)
    t1 = (lost.item_name or "").lower().strip()
    t2 = (found.item_name or "").lower().strip()
    if t1 and t2:
        if t1 == t2:
            title_score = 30.0
            ...
            title_score = round(overlap * 30.0, 1)
    else:
        title_score = 15.0
        
    score_breakdown["item_name"] = title_score

    # Gatekeeper: if item name has zero overlap, likely not a match
    if title_score < 5.0:
        score_breakdown["incompatibility"] = f"Item Name mismatch: '{t1}' vs '{t2}'"
        return 0.0, score_breakdown

    # 2. Brand & Model (15 pts)
    ...
    # 6. Description & Details Overlap (5 pts)
    ...
    text_subtotal = title_score + brand_score + color_score + location_score + date_score + desc_score

    # 7. Image Similarity & Fallback (20 pts)
